...
### Notice for Anchore with hardened.enabled
If you choose to set `istio.hardened.enabled` for Anchore, you will need to add `customServiceEntries` for each registry you wish to scan images in (see below).  For example, if you wanted to scan images in registry1, you would need to add a customServiceEntry for `registry1.dso.mil`, and another for the S3 bucket that Harbor redirects to when downloading image layers (currently `ib-prod-harbor-storage.s3.us-gov-west-1.amazonaws.com`). For `docker.io/library/centos:latest`, the following were required during testing: `docker.io`, `registry-1.docker.io`, `auth.docker.io`, `production.cloudflare.docker.com`. Keep in mind the location image layers come from may differ by image within a given registry.

Additionally, any custom vulnerability feeds you wish to use with Anchore will also need a customServiceEntry, or, alternatively, you can add the host for those vulnerability feeds to the `feedsHosts` key.  The `feedsHosts` key is already populated with all of the feeds that are configured with Anchore Enterprise by default.
